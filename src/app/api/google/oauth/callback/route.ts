...
export async function GET(req: Request) {
  ...
  if (!clientId || !clientSecret || !redirectUri) {
    return NextResponse.json({ ok: false, error: "Missing GOOGLE env vars" }, { status: 500 });
  }
  ...
  if (!refreshToken) {
    return NextResponse.json(
      { ok: false, error: "No refresh_token returned. Revogue o acesso do app e refaça o consent." },
      { status: 400 }
    );
  }
